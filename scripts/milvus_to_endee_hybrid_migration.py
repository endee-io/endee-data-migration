            ...
            while not self.interrupted:
                try:
                    ...
                    if batch_number == 0 and records:
                        ...
                        if ENDEE_SPARSE_INDICES_KEY in sample and len(sample[ENDEE_SPARSE_INDICES_KEY]) > 5:
                            sample[ENDEE_SPARSE_INDICES_KEY] = f"{sample[ENDEE_SPARSE_INDICES_KEY][:5]}... ({len(sample[ENDEE_SPARSE_INDICES_KEY])} items)"
                        if ENDEE_SPARSE_VALUES_KEY in sample and len(sample[ENDEE_SPARSE_VALUES_KEY]) > 5:
                            sample[ENDEE_SPARSE_VALUES_KEY] = f"{sample[ENDEE_SPARSE_VALUES_KEY][:5]}... ({len(sample[ENDEE_SPARSE_VALUES_KEY])} items)"
